...
def sonne():
    s = '<g stroke="#f6bc68">'
    s += '<circle cx="0" cy="0" r="18" fill="none" />'
    s += '<path d="'
    for i in range(8):
        w = math.pi*i/4
        ri = 24
...
def mond():
    s = '<path stroke="#da4935" fill="none" d="M 0,-24 a 26,26 0 0 1 -22,39 a 24,24 0 1 0 22,-39 z" />'
    return s
...
def bewoelkt(wolke=1,mit_sonne=False,mit_mond=False):
    ...
    if mit_sonne and wolke<4:
        ...
        s += '<g stroke="#f6bc68">'
        if not arc:
            s += '<circle cx="%s" cy="%s" r="%s" fill="none" />' % (cx,cy,r)
        s += '<path fill="none" d="'
        if arc:
